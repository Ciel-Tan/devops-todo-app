...
@Field
String DOCKER_USER_REF = 'Tan_dockerhub'
...
pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                sh 'docker build -t cieltan/npm-training-todoapp:0.0.2 .'
            }
        }
        stage("Docker login and push docker image") {
		    ...
		    sh 'docker login --username ${repository_username} --password ${repository_password}'                  
                    sh 'docker push cieltan/npm-training-todoapp:0.0.2'        		
                }
            }
        }
        stage("deploy") {
             steps {
                 withBuildConfiguration {
                     sshagent(credentials: [SSH_ID_REF]) {
                         sh '''
                            ssh -o StrictHostKeyChecking=no root@ec2-18-143-167-76.ap-southeast-1.compute.amazonaws.com "docker run --detach --name tan-todo-app -p 33333:8000 cieltan/npm-training-todoapp:0.0.2"
                            docker ps -a
                            '''
                    }
                }
